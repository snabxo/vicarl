...
static void make_tmp_db(char out[256]) {
#ifdef _WIN32
    snprintf(out, 256, "vicarl_test.db");
#else
    snprintf(out, 256, "/tmp/vicarl_test_%d.db", (int)getpid());
#endif
}
...
void test_store_sqlite(void) {
    ...
    opt.sqlite_wal = 1;
    opt.sqlite_synchronous = 1;

    ASSERT_ST_OK(vicarl_store_open_sqlite(&st, db, &opt));
    ASSERT_TRUE(st != NULL);

    // tip should be NOT_FOUND initially
    uint64_t tip_no = 0;
